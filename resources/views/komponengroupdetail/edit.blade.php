...
<!-- <div class="nk-fmg-body-content"> -->
    <div class="nk-fmg-quick-list nk-block">
        <div class="card">
            <div class="card-body">
                <form id="form" action="{{ route('komponengroupdetail.update', $data->id)}}" method="POST" enctype="multipart/form-data">
                    @csrf
                <div class="mb-3">
                    <label for="exampleFormControlInput1" class="form-label">Komponen Detail</label>
                    <input class="form-control" id="group" name="gkomponen_detail"  value="{{ $data->gkomponen_detail }}" required>
                  </div>
                <div class="mb-3">
                    <label for="sel1" class="form-label">Group Komponen :</label>
                    <select class="form-control" id="goldarah" name="gkomponen_id" required>
                        @foreach($gkomponen as $gk)
                            <option value="{{$gk->id}}"
                                @if($gk->id == $data->gkomponen_id)
                                    SELECTED
                                    @endif
                            >{{$gk->group}}</option>
                        @endforeach
