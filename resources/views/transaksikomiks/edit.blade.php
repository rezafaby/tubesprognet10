...
<!-- <div class="nk-fmg-body-content"> -->
    <div class="nk-fmg-quick-list nk-block">
        <div class="card">
            <div class="card-body">
                <form id="form" action="{{ route('transaksikomiks.update', $data->id)}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                    ...
                    <select class="form-control" id="nama_iks" name="nama_iks" required>
                        @foreach($tikspro as $tip)
                            <option value="{{$tip->id}}"
                                @if($tip->id == $data->iks_provider_id)
                                    SELECTED
                                    @endif
                            >{{$tip->nama_iks}}</option>
                        @endforeach
